...
@NamedQueries({
   @NamedQuery(name="ServiceFileEntity.findAll", query="SELECT s FROM ServiceFileEntity s"),
   @NamedQuery(name="ServiceFileEntity.findByClientService", query="SELECT s FROM ServiceFileEntity s WHERE s.clientService=:clientService")
})
...
public class ServiceFileEntity implements Serializable {
	...
	@Column(name="id_service_file")
	private String idServiceFile;
	...
	public String getIdServiceFile() {
		return this.idServiceFile;
	}

	public void setIdServiceFile(String idServiceFile) {
